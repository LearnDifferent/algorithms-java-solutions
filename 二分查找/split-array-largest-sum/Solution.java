...
我们可以反向思维，将可能出现的最大值，当作一个有序数组，在这个"最大值集合"的有序数组中，
查找在"固定最大值"的情况下，在每个"固定的最大值"约束下，该数组"至少可以切分为多少个子数组"。
（也就是说，每个子数组都尽可能接近"固定的最大值"，看看能分为多少个子数组）

当切分的子数组的数量，符合题目要求的 m 时，再换一个"更小的最大值"，也就是寻找左侧边界。
（在多种排列后的最大值中，找到可以分为 m 组的情况，然后在符合的情况中，选择左侧边界）
...
class Solution {
    ...
    public int splitArray(int[] nums, int m) {
        ...
        // 就是 nums 中每个元素都单独为 1 组，这样 nums 中的最大元素，肯定就是所有情况中，"最小的最大值"。
        // 在 非每个元素都单独为 1 组的情况下，最大的那个元素肯定会和别的元素相加，
        // 所以只有最大的元素单独为一组才是最小的"最大值"。
        int left = getMaxInNums(nums);

        // 而最大的最大值，就是全部 nums 中元素为一个分组
        ...
        while (left < right) {
            ...
            if (count < m) {
                // 最大值"太大了，导致分组少于 m，此时，需要更小的"最大值"以增加分组数
                // 所以去左侧的 [left, mid) 中，找更小的最大值
                right = mid;
            } else if (count > m) {
